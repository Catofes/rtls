...
`WebListen`为管理 HTTP 服务的监听地址；省略或设为空字符串时不启动。服务提供`GET /healthz`健康检查，以及`GET /logs`日志查询；日志查询可使用`n`返回最后 N 条，或使用`q`按不区分大小写的文本筛选。

`IdleTimeout`为代理隧道单个方向的空闲读取超时，单位为秒；`0`表示不设置空闲超时。`LogBufferLen`为内存中保留的日志行数；`0`或负数表示不保留内存日志。
